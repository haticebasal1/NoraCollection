...
namespace NoraCollection.API.Controllers
{
    ...
    public class ProductImagesController : CustomControllerBase
    {
        ...
        {
            var response = await _productImageService.GetByProductIdAsync(productId);
            return CreateResult(response);
        }
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(int id)
        {
            var response = await _productImageService.GetAsync(id);
            return CreateResult(response);
        }
        [HttpPost]
        public async Task<IActionResult> Add([FromForm] ProductImageCreateDto productImageCreateDto)
        {
            var response = await _productImageService.AddAsync(productImageCreateDto);
            return CreateResult(response);
        }
        [HttpPut]
        public async Task<IActionResult> Update([FromForm] ProductImageUpdateDto productImageUpdateDto)
        {
            var response = await _productImageService.UpdateAsync(productImageUpdateDto);
            return CreateResult(response);
        }
        [HttpPatch("set-main/{id}")]
        public async Task<IActionResult> SetAsMain(int id)
        {
            var response = await _productImageService.SetAsMainAsync(id);
            return CreateResult(response);
        }
        [HttpPatch("display-order/{id}")]
        public async Task<IActionResult> UpdateDisplayOrder(int id, [FromBody] int displayOrder)
        {
            var response = await _productImageService.UpdateDisplayOrderAsync(id, displayOrder);
            return CreateResult(response);
        }
        [HttpDelete("soft-delete/{id}")]
        public async Task<IActionResult> SoftDelete(int id)
        {
            var response = await _productImageService.SoftDeleteAsync(id);
            return CreateResult(response);
        }
        [HttpDelete("hard-delete/{id}")]
        public async Task<IActionResult> HardDelete(int id)
        {
            var response = await _productImageService.HardDeleteAsync(id);
            return CreateResult(response);
        }
    }
}
